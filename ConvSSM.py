...
from timm.layers import get_padding, SelectAdaptivePool2d
from modules import *
...
class SS2D(
    ...
    def __init__(
        ...
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0))  # (K=4, N, inner) 将四个相同配置的全连接层初始化，并将它们的权重合并成一个单一的可训练参数
        del self.x_proj #删除原始的之前初始化的x_proj
...
class ConvSSM(nn.Module):
    ...
    def __init__(
        ...
        # self.self_attention = RFAConv(in_channel=hidden_dim // 2, out_channel=hidden_dim // 2)
        ...
        self.intDW = InceptionDWConv2d(in_chs=hidden_dim // 2)
        self.norm = CustLayerNorm(hidden_dim // 2)
    ...
    def forward(self, input: torch.Tensor, mask=None):
        ...
        input_left = self.act(self.norm(self.intDW(input_left))) + input_left
        x = x.permute(0, 3, 1, 2).contiguous()
        output = torch.cat((input_left, x), dim=1)
        output = self.finalconv11(output).permute(0, 2, 3, 1).contiguous()
        # return (output + input).reshape(B, N, C)
        return (output + input)
